...
class BlackJack:
    def __init__(self):
        ...
        self.optimal_policy = np.zeros((10,10,2),dtype = np.int)
    ...
    def updatePolicy(self,usable_ace,player_state,dealer_card,st_act_vals,st_act_cnt):
        #print(st_act_vals[player_state, dealer_card, usable_ace, :] )
        # get argmax of the average returns(s, a)
        usable_ace = int(usable_ace)
        values = st_act_vals[player_state, dealer_card, usable_ace, :] / st_act_cnt[player_state, dealer_card, usable_ace, :]
        #print('values')
        #print(values)
        max_val = np.max(values)
        #print('max_val:',max_val)
        temp_list = []
        for action, val in enumerate(values):
            #print('cmn')
            if max_val == val:
                #print('cmn2')
                temp_list.append(action)
        self.optimal_policy[player_state, dealer_card, usable_ace] = np.random.choice(temp_list)#np.argmax(values)
    
  
    def getOptimalPolicyAction(self,usable_ace,player_state,dealer_card):
        player_state -= 12
        dealer_card -= 1
        usable_ace = int(usable_ace)
        return self.optimal_policy[player_state, dealer_card, usable_ace]
    ...
    def setGameInitStatus(self,init_state, player_state,pusable_ace_flag,dealer_card1,dealer_card2,dusable_ace_flag):
        ...
        if init_state is None:
            ...
            while player_state < 12:
                #if player state < 12, always hit
                card, card_value = self.drawCardAndValue()
                player_state += card_value

                #if the player state > 21, he may hold one or two aces.
                if player_state > 21:
    ...
    def playerPlays(self,init_action,player_state,pusable_ace_flag,player_seq_plays,dealer_card1,episode):
        ...
        while True:
            if init_action is not None:
                ...
                if episode:
                    action = self.getOptimalPolicyAction(pusable_ace_flag, player_state, dealer_card1)
                else:
                    action = self.getPlayerPolicySA(player_state)
    ...
    def playGame(self, episode = 0, init_state = None, init_action = None):
        ...
        player_state, player_seq_plays = self.playerPlays(init_action, player_state, pusable_ace_flag, player_seq_plays, dealer_card1,episode)
        if player_state == -1:
    ...
    def playGame(self, episode, init_state = None, init_action = None):
        #initially player's status
        #player state, player sequence of plays, whether he has usable ace 
        player_state, player_seq_plays, pusable_ace_flag = 0 , [], False
